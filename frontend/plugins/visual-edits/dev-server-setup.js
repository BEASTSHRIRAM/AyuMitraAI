...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all ayumitra subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*ayumitra\.sh$/)) {
        return true;
      }

      // Allow all ayumitra.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*ayumitra\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@ayumitra.com" add "${targetFile}"`);;
          } catch (gitError) {
